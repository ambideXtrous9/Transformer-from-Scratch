# DecoderMoEMLAInference.py Module Documentation

## 1. Overview

This script performs **text generation (inference)** using a trained `DecoderOnlyMoEMLAModel`. It loads the latest PL checkpoint and uses **greedy decoding** to generate answers from GSM8K-style prompts.
...
graph TD
    ...
    subgraph "Greedy Decoding Loop"
        Loop[Current Sequence] --> Model[DecoderOnlyMoEMLAModel]
...
```bash
python PLTrainerScripts/DecoderMoEMLAInference.py
```

Requires a trained checkpoint in `checkpoints/DecoderMoEMLACheckpoints/`.

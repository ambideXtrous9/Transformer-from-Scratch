"""
HF Trainer — Decoder-Only with MoE + MLA on GSM8K
"""
...
OUTPUT_DIR = config.CHECKPOINTS["hf_moe_mla"]
...
if __name__ == "__main__":
    ...
    base_model = DecoderOnlyMoEMLAModel(
        vocab_size=vocab_size, d_model=config.D_MODEL, max_positions=MAX_LENGTH,
        num_layers=config.NUM_LAYERS, num_heads=config.NUM_HEADS, d_compress=config.D_COMPRESS,
    ...
    os.environ["WANDB_NAME"] = "HF-DecoderOnly-MoE-MLA"
    ...
    print("\n" + "=" * 50)
    print("Starting Training — DecoderOnly (MoE + MLA)")
    print("=" * 50 + "\n")

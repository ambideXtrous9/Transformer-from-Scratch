...
# ---------------- Decoder Block (GPT-style with GQA) ----------------
class DecoderBlock(nn.Module):
    def __init__(self, d_model: int = 256, num_heads: int = 8, num_kv_heads: int = 2, d_ff: int = 1024, dropout: float = 0.1):
        super().__init__()
        # 1. Masked Self-Attention (Group Query Attention)
        self.mhsa = GroupQueryAttention(d_model=d_model, num_heads=num_heads, num_kv_heads=num_kv_heads, dropout=dropout, causal=True)
...
# ---------------- Decoder-only Transformer with GQA ----------------
class DecoderOnlyGQAModel(pl.LightningModule):
    def __init__(
        ...
        self.layers = nn.ModuleList([
            DecoderBlock(d_model=d_model, num_heads=num_heads, num_kv_heads=num_kv_heads, d_ff=d_ff, dropout=dropout)

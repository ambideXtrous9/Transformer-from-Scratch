...
# ---------------- Decoder Block: MLA attention + MoE FFN ----------------
class DecoderBlockMoEMLA(nn.Module):
    def __init__(self, d_model=256, num_heads=8, d_compress=64, d_ff=1024,
                 dropout=0.1, num_experts=4, top_k=2):
        super().__init__()
        self.mhsa = MultiHeadLatentAttention(d_model=d_model, num_heads=num_heads,
                                              d_compress=d_compress, dropout=dropout, causal=True)
...
# ---------------- Decoder-Only Transformer: MoE + MLA ----------------

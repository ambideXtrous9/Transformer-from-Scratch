...
# ---------------- Decoder Block (GPT-style with MQA) ----------------
class DecoderBlock(nn.Module):
    def __init__(self, d_model: int = 256, num_heads: int = 8, d_ff: int = 1024, dropout: float = 0.1):
        super().__init__()
        # 1. Masked Multi-Query Attention
        self.mhsa = MultiQueryAttention(d_model=d_model, num_heads=num_heads, dropout=dropout, causal=True)
    ...
    def forward(self, x: torch.Tensor, tgt_mask: Optional[torch.Tensor] = None):
        # 1. Masked Multi-Query Attention + AddNorm
...
# ---------------- Decoder-only Transformer with MQA ----------------

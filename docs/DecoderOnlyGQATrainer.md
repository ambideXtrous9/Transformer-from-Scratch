# DecoderOnlyGQATrainer.py Module Documentation

## 1. Overview

This script handles the **training pipeline** for the `DecoderOnlyGQAModel` (Group Query Attention). It mirrors `DecoderOnlyTrainer.py` but trains a model that uses Group Query Attention instead of standard Multi-Head Attention, sharing KV heads across multiple query heads.
...
-   `DecoderOnlyGQAModel.py` -> `DecoderOnlyGQAModel`: The model being trained.
...
graph TD
    ...
    Trainer --> Model[DecoderOnlyGQAModel]
    Trainer --> CK[Checkpoint]
    CK --> Disk[GQACheckpoints/]
```
...
4.  **Input IDs**: `[BOS] + token_ids`, padded to `max_length` with `pad_token_id`.
5.  **Labels**: `token_ids + [EOS]`, padded with `-100`.
...
```bash
cd GQA && python DecoderOnlyGQATrainer.py
```

Requires `versatile_dataset_2000.csv` in the parent directory (loaded via `os.path.join`).

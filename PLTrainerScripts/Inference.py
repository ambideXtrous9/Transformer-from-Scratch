...
if __name__ == "__main__":
    ...
    model = load_latest_checkpoint(config.CHECKPOINTS["seq2seq"], vocab_size, tokenizer)

    questions = config.SAMPLE_QUESTIONS

    for q in questions:
        src_text = f"Question: {q}"
        output = greedy_decode(model, tokenizer, src_text, max_len=config.MAX_LENGTH)
        print("\n" + "=" * 60)
        print(f"Question: {q}")
        print(f"\nGenerated Answer:\n{output}")
        print("=" * 60)

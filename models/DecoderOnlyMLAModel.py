...
from core.attention.MultiHeadLatentAttention import MultiHeadLatentAttention
...
# ---------------- Decoder Block (GPT-style with MLA) ----------------
class DecoderBlock(nn.Module):
    def __init__(self, d_model: int = 256, num_heads: int = 8, d_compress: int = 64, d_ff: int = 1024, dropout: float = 0.1):
        super().__init__()
        # 1. Masked Self-Attention (Multi-Head Latent Attention)
        self.mhsa = MultiHeadLatentAttention(d_model=d_model, num_heads=num_heads, d_compress=d_compress, dropout=dropout, causal=True)
...
# ---------------- Decoder-only Transformer with MLA ----------------
class DecoderOnlyMLAModel(pl.LightningModule):
    def __init__(
        ...
        self.layers = nn.ModuleList([
            DecoderBlock(d_model=d_model, num_heads=num_heads, d_compress=d_compress, d_ff=d_ff, dropout=dropout)

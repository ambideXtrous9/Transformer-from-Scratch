...
if __name__ == '__main__':
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath = config.CHECKPOINTS["moe_mla"],
        filename = 'DecoderMoEMLABestModel',
        save_top_k = 1,
        verbose = True,
    ...
    # ---------------- Trainer ----------------
    wandb_logger = WandbLogger(project=config.WANDB_PROJECT, name="DecoderOnly-MoE-MLA", log_model=False)
    wandb_logger.experiment.config.update({
        "architecture": "DecoderOnly-MoE-MLA",
        "d_model": config.D_MODEL,
        "num_layers": config.NUM_LAYERS,
        "num_heads": config.NUM_HEADS,
        "d_compress": config.D_COMPRESS,
